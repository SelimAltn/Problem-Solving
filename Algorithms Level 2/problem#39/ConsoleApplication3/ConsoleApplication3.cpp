...
#include <ctime>
using namespace std;
enum ENasalmıdegilmi { asal = 1, asaldeğil = 2};
...
ENasalmıdegilmi kontrolnoktası(int number)
{
    int m = round(number / 2);
    for (int x = 2; x <= m; x++)
    {
        if (number % x == 0)
            return ENasalmıdegilmi::asaldeğil;
    }
    return ENasalmıdegilmi::asal;
...

{
    ...
    {
        if (kontrolnoktası(arri1[i])==ENasalmıdegilmi::asal)
        {
            arri2[j] = arri1[i];
                j++;
        }
